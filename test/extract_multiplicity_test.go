...
var testCases map[string]MultTestCaseRes = map[string]MultTestCaseRes{
	...
	"*":                              {HasMult: true, Expected: Multiplicity{Start: -1, HasEndMult: false}, ErrMsg: "'Many' should be translated to '-1'"},
	"*..*":                           {HasMult: true, Expected: Multiplicity{Start: -1, HasEndMult: false}, ErrMsg: "Many to many should be normalised!"},
